...
var UserWhere = struct {
	...
	DiscordID: whereHelperstring{field: "\"users\".\"discord_id\""},
}
...
type userR struct {
	DMCampaigns      CampaignSlice        `boil:"DMCampaigns" json:"DMCampaigns" toml:"DMCampaigns" yaml:"DMCampaigns"`
...
var (
	userAllColumns            = []string{"id", "created_at", "updated_at", "deleted_at", "discord_id"}
	userColumnsWithoutDefault = []string{"created_at", "updated_at", "deleted_at", "discord_id"}
...
// DMCampaigns retrieves all the campaign's Campaigns with an executor via dm_id column.
func (o *User) DMCampaigns(mods ...qm.QueryMod) campaignQuery {
...
func (o *User) PlayerCharacters(mods ...qm.QueryMod) playerCharacterQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"player_characters\".\"user_id\"=?", o.ID),
	)
...
// LoadDMCampaigns allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadDMCampaigns(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`campaigns`),
		qm.WhereIn(`campaigns.dm_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on campaigns")
	}
	...
	if singular {
		object.R.DMCampaigns = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.DMID {
				local.R.DMCampaigns = append(local.R.DMCampaigns, foreign)
				if foreign.R == nil {
					foreign.R = &campaignR{}
				}
				foreign.R.DM = local
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadPlayerCharacters(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`player_characters.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// AddDMCampaigns adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.DMCampaigns.
// Sets related.R.DM appropriately.
func (o *User) AddDMCampaigns(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Campaign) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.DMCampaigns = append(o.R.DMCampaigns, related...)
	}
...
func (o *User) AddPlayerCharacters(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*PlayerCharacter) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
