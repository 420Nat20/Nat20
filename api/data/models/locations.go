...
type Location struct {
	...
	CampaignID       int         `boil:"campaign_id" json:"campaign_id" toml:"campaign_id" yaml:"campaign_id"`
...
var LocationWhere = struct {
	...
	EventDescription: whereHelpernull_String{field: "\"locations\".\"event_description\""},
	Visited:          whereHelpernull_Bool{field: "\"locations\".\"visited\""},
	CampaignID:       whereHelperint{field: "\"locations\".\"campaign_id\""},
}
...
type locationR struct {
	...
	SubLocations SubLocationSlice `boil:"SubLocations" json:"SubLocations" toml:"SubLocations" yaml:"SubLocations"`
}
...
var (
	locationAllColumns            = []string{"id", "updated_at", "created_at", "name", "event_description", "visited", "campaign_id"}
	locationColumnsWithoutDefault = []string{"updated_at", "created_at", "name", "event_description", "campaign_id"}
	locationColumnsWithDefault    = []string{"id", "visited"}
...
func (o *Location) Campaign(mods ...qm.QueryMod) campaignQuery {
...
// SubLocations retrieves all the sub_location's SubLocations with an executor.
func (o *Location) SubLocations(mods ...qm.QueryMod) subLocationQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"sub_locations\".\"location_id\"=?", o.ID),
	)

	query := SubLocations(queryMods...)
	queries.SetFrom(query.Query, "\"sub_locations\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"sub_locations\".*"})
	}
...
func (locationL) LoadCampaign(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLocation interface{}, mods queries.Applicator) error {
	var slice []*Location
	var object *Location
...
// LoadSubLocations allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (locationL) LoadSubLocations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeLocation interface{}, mods queries.Applicator) error {
	var slice []*Location
	var object *Location
	...
	query := NewQuery(
		qm.From(`sub_locations`),
		qm.WhereIn(`sub_locations.location_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.QueryContext(ctx, e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load sub_locations")
	}

	var resultSlice []*SubLocation
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice sub_locations")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on sub_locations")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for sub_locations")
	}
	...
	if singular {
		object.R.SubLocations = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &subLocationR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.LocationID {
				local.R.SubLocations = append(local.R.SubLocations, foreign)
				if foreign.R == nil {
					foreign.R = &subLocationR{}
				}
...
// SetCampaign of the location to the related item.
// Sets o.R.Campaign to related.
// Adds o to related.R.Locations.
func (o *Location) SetCampaign(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Campaign) error {
	var err error
	if insert {
...
// AddSubLocations adds the given related objects to the existing relationships
// of the location, optionally inserting them as new records.
// Appends related to o.R.SubLocations.
// Sets related.R.Location appropriately.
func (o *Location) AddSubLocations(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*SubLocation) error {
	var err error
	...
	if o.R == nil {
		o.R = &locationR{
			SubLocations: related,
		}
	} else {
		o.R.SubLocations = append(o.R.SubLocations, related...)
	}
