...
async def enter(ctx):
    ...
    for temp in roles:
        if '-' not in temp.name and temp.name != '@everyone':
            location = temp
            break

    for temp in roles:
        if temp.name == f'{location.name}-{sublocation}':
            role = temp
            break
    ...
    if role is None:
        await create_role(ctx, f'{location.name}-{sublocation}')
        await create_channel(ctx, f'{location.name}-{sublocation}')
    else:
